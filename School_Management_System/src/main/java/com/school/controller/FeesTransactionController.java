...
public class FeesTransactionController {
    ...
    public Map<String, Object> getFeesTransactions(@PathVariable String institution_id) {
        ...
        if (allFeesTransactions.isEmpty()) {
            return JsonResponses.generateResponse1(false, allFeesTransactions, "Institution Id is Invalid Or List is Empty");
        } else {
            return JsonResponses.generateResponse1(true, allFeesTransactions, "Fees Transactions Details Get Successfully");
        }
    }
    ...
    public Map<String, Object> addFeesTransaction(@PathVariable String institute_id,
        ...
        boolean savedFeesTransaction = feesTransactionService.saveFeesTransaction(feesTransactionRequest, institute_id);

        if (savedFeesTransaction) {
    ...
    public Map<String, Object> updateFeesTransactionById(@PathVariable String institute_id, @PathVariable int fee_id,
        ...
        boolean updatedFeesTransaction = feesTransactionService.updateFeesTransaction(feesTransactionRequest, institute_id, fee_id);

        if (updatedFeesTransaction) {
            return JsonResponses.generateResponse1(true, feesTransactionRequest, "Fees Transaction Data Updated Successfully");
        } else {
    ...
    public Map<String, Object> findFeesTransactionById(@PathVariable String institute_id, @PathVariable int fee_id) {
        Optional<FeesTransaction> feesTransaction = feesTransactionService.findFeesTransactionById(fee_id, institute_id);
